 ...
 * project CIS*2750F13 (implementation)
 *
 ...
 * handles network connectivity and transmission of request/Codemon (report
 * details TBD as this will all be asynchronous on the server-side)
 *
 * @pre  for Codemon: 0 < lines <= MAXINST
 * @pre  for Codemon: 0 <= begin < lines
 * @pre  for Codemon: inst_buf contains lines instructions
 ...
 * @pre  for Codemon: 0 <= begin < lines
 * @pre  for Codemon: inst_buf contains lines instructions
 * @post cmon has been run on Codex in TEST mode for limit iterations
 * @post report received and stored in static buffer
 *
 ...
 * followed by the program (contiguous sequence of lines uint32).  Note
 * that all integral values are sent in network byte order (including
 * the program itself).  Expects an ACK from the server to confirm receipt.
 *
...
int32 _sendCodemon(struct Codemon_pkg *cmon, FILE *fp)
{
    ...
    uint32 netbuf[MAXINST];     /* buffer for network byte order conversion */
    int32 i;                    /* loop index */

    netlines = htonl(cmon->lines);
    netbegin = htonl(cmon->begin);
    for (i = 0; i < cmon->lines; i++)
        netbuf[i] = htonl(cmon->program[i]);
    ...
    if (fwrite(&netbegin, sizeof(uint32), 1, fp) < 1)
        return(0);
    if (fwrite(netbuf, sizeof(uint32), cmon->lines, fp) < cmon->lines)
        return(0);
